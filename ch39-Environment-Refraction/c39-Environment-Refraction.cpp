...
void render()
{
	...
	glm::mat4 model_matrix = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -8.0f, -15.0f))
		* glm::rotate(glm::mat4(1.0f), glm::radians(time * 30.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	proj_matrix = glm::perspective(45.0f, 720.0f / 640.0f, 0.1f, 100.0f);
	mvp_matrix = proj_matrix * view * model_matrix;
	glUniformMatrix4fv(cube_model_loc, 1, GL_FALSE, &model_matrix[0][0]);
	glUniformMatrix4fv(cube_mvp_loc, 1, GL_FALSE, &mvp_matrix[0][0]);
	glActiveTexture(GL_TEXTURE0);
	glUniform1i(cube_tex_loc, 0);
	glBindTexture(GL_TEXTURE_CUBE_MAP, cubemap_texture);
	model.Draw(cube_program);
	glBindVertexArray(0);
