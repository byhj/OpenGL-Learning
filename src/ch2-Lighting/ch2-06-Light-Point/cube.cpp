...
void Cube::Render(const const byhj::MvpMatrix &matrix, const byhj::Camera camera)
{
	...
	glUniform3fv(lightPosLoc, 1,  &glm::vec3(1.5f, 0.5f, 0.0f)[0]);
	glUniform1f(glGetUniformLocation(program, "light.constant"), 1.0f);
	glUniform1f(glGetUniformLocation(program, "light.linear"), 0.045);
	glUniform1f(glGetUniformLocation(program, "light.quadratic"), 0.045);
